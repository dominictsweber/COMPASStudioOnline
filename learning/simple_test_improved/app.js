...
const App = {
                ...
                lines.forEach((line, index) => {
                    ...
                    while ((sMatch = regexSwitch.exec(line)) !== null) {
                        ...
                        const createBtn = (text, dir) => {
                             ...
                             btn.style.margin = '0';
                             // Borders
                             if(dir === -1) btn.style.borderRight = '1px solid #ccc';
                             if(dir === 1) btn.style.borderLeft = '1px solid #ccc';
                        ...
                        const arrowWidth = 20;
                        const totalWidth = (labelWidth) + (arrowWidth * 3);
    ...
    async runCode(path, node) {
        ...
        try {
            // Gather imports
            const imports = {
                compas_geo: document.getElementById('chk-compas-geo')?.checked,
                compas_ds: document.getElementById('chk-compas-ds')?.checked,
                Point: document.getElementById('chk-pt')?.checked,
                Vector: document.getElementById('chk-vec')?.checked,
                Box: document.getElementById('chk-box')?.checked,
                Frame: document.getElementById('chk-frame')?.checked,
                Mesh: document.getElementById('chk-mesh')?.checked
            };

            const response = await fetch('/execute', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ filename: path, code, imports })
            });
