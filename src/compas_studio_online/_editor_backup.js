...
const App = {
    ...
    init() {
        ...
        if (typeof io !== 'undefined') {
            ...
            this.socket.on('code_update', (data) => {
                ...
                // Note: deeply nested files need a better finding mechanism than this flat assumption
                // but currently loadProjectFiles only supports flat structure anyway.
        ...
        let projectName = window.location.pathname.substring(1);
        if (projectName.endsWith('/')) projectName = projectName.slice(0, -1);
        
        // Fallback for query param (or non-numeric paths which might be routed differently)
        if (!projectName || !projectName.match(/^\d+$/)) {
        ...
        if (projectName) {
            ...
            // Join Socket Room
            if (this.socket) this.socket.emit('join', { project: projectName });
            
            // Load file structure from server
            this.loadProjectFiles(projectName);
        } else {
             // Fallback to memory
             this.state.currentProjectName = "Default Project";
             // Render hardcoded explorer if no project (or maybe just empty)
             this.renderExplorer();
        }
        ...
        // Warmup
        this.warmupKernel();
        // Imports are run after files loaded
    },
    ...
    renderToolbar(wrapper) {
        ...
        backBtn.onclick = () => { window.location.href = '/'; }; // Go back to index
    ...
    async runCode(path, node) {
        ...
        try {
            const response = await fetch('/execute', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ filename: path, code, pre_import_code: importsCode })
            });
